-- Migración 028: deuda de proveedor (marcas) atómica + fix tabla renombrada
-- 1. RPC ajustar_deuda_marca: ajuste atómico de deuda_total (evita race condition
--    del patrón read+add+write desde el cliente)
-- 2. Recrea procesar_venta y procesar_cobro_deuda corrigiendo "proveedores" → "marcas"
--    (la tabla se renombró en la migración 015; estos RPCs quedaron rotos en la
--    rama de pago a proveedor: "relation proveedores does not exist")

-- ───────────────────────────────────────────────────────────────────────────
-- 1. Ajuste atómico de deuda de marca (proveedor)
-- ───────────────────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION ajustar_deuda_marca(p_marca_id UUID, p_delta INTEGER)
RETURNS INTEGER LANGUAGE sql SECURITY DEFINER AS $$
  UPDATE marcas
  SET deuda_total = GREATEST(0, deuda_total + p_delta)
  WHERE id = p_marca_id
  RETURNING deuda_total;
$$;

GRANT EXECUTE ON FUNCTION ajustar_deuda_marca(UUID, INTEGER) TO authenticated;

-- ───────────────────────────────────────────────────────────────────────────
-- 2. procesar_venta — idéntica a 022, sólo cambia "proveedores" → "marcas" (L120)
-- ───────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  IF p_proveedor_id IS NOT NULL AND p_monto_proveedor > 0 THEN
    ...
    UPDATE marcas SET deuda_total = GREATEST(0, deuda_total - p_monto_proveedor) WHERE id = p_proveedor_id;
  END IF;
...
-- ───────────────────────────────────────────────────────────────────────────
-- 3. procesar_cobro_deuda — idéntica a 024, sólo cambia "proveedores" → "marcas" (L35)
-- ───────────────────────────────────────────────────────────────────────────
