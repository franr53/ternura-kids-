-- Defensive fix: use ROUND(::NUMERIC)::INTEGER so decimal strings like "16000.00"
-- don't crash the cast. The TypeScript layer already sends Math.round() values,
-- but this guards against future callers.
...
BEGIN
  ...
  FOR v_pago IN SELECT * FROM jsonb_array_elements(
    ...
    v_monto := ROUND((v_pago->>'monto')::NUMERIC)::INTEGER;
