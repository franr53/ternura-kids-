...
export default function EtiquetasPage() {
  ...
  const { data: _productos } = useCache<ProductoConProveedor[]>('etiq:prods', async () => {
  ...
  function buildEtiquetaDataList(): EtiquetaData[] {
    ...
    etiquetas.forEach(e => {
      ...
      const precioLista = e.variante.precio_venta ?? producto?.precio_venta ?? 0
      const precioEfectivo = Math.round(precioLista * 0.8)
      for (let i = 0; i < e.cantidad; i++) {
        result.push({
          nombre: producto?.nombre || '',
          marca: producto?.proveedor?.nombre || '',
                  ...
                  <div key={producto.id}>
                    ...
                    {variantesFiltradas.map(v => (
                      ...
                      >
                        ...
                        <span className="font-medium text-gray-700">{formatPrecio(v.precio_venta ?? producto.precio_venta)}</span>
                      </button>
                    ))}
                  </div>
                )
              ...
              {etiquetas.map(e => (
                <div key={e.variante_id} className="flex items-center gap-4 py-2 border-b border-gray-50 last:border-0">
                  <div className="flex-1">
                    <p className="font-medium text-gray-800">{formatNombreConTalle(e.variante.producto?.nombre || '', e.variante.talle)}</p>
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      <span className="text-sm text-gray-500">
                        {formatPrecio(e.variante.precio_venta ?? e.variante.producto?.precio_venta ?? 0)}
                        <span className="text-xs text-gray-400 ml-1">
                          · ef. {formatPrecio(Math.round((e.variante.precio_venta ?? e.variante.producto?.precio_venta ?? 0) * 0.8))}
                        </span>
                      </span>
