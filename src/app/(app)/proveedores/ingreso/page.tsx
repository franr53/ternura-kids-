...
function parsearLista(texto: string, productos: (Producto & { variantes?: Variante[] })[]): ItemEscaneado[] {
  ...
  for (const linea of lineas) {
    ...
    for (const p of productos) {
      const score = fuzzyScore(p.nombre_base, nombre)
...
function IngresoMercaderiaContent() {
  ...
  function seleccionarProducto(producto: Producto & { variantes?: Variante[] }) {
    ...
    setBusquedaProducto('')
    setVarianteId('')
    setPrecioCostoItem(producto.variantes?.[0]?.precio_costo?.toString() || '')
  ...
  function confirmarEscaneados() {
    ...
    for (const item of itemsEscaneados) {
      ...
      setItems(prev => [...prev, {
        ...
        precio_costo: item.precio_unitario ?? prod.variantes?.[0]?.precio_costo ?? 0,
      }])
      agregados++
    }
                  ...
                  <div className="grid grid-cols-4 gap-2 items-end">
                    <div className="col-span-2">
                      ...
                      >
                        ...
                        {productos.map(p => <option key={p.id} value={p.id}>{p.nombre_base}</option>)}
                      </select>
                    </div>
                    <div>
        ...
        <CardContent className="space-y-4">
          <div className="relative">
            ...
            <Input
              ...
              value={productoSeleccionado ? productoSeleccionado.nombre_base : busquedaProducto}
            ...
            {productosFiltrados.length > 0 && !productoSeleccionado && (
              <div className="absolute top-full left-0 right-0 bg-white border border-gray-200 rounded-lg shadow-lg z-10 max-h-48 overflow-y-auto mt-1">
                {productosFiltrados.map(p => (
                  ...
                  >
                    <span className="font-medium">{p.nombre_base}</span>
              ...
              <tbody>
                {items.map(item => (
                  <tr key={item.variante_id} className="border-b border-gray-50 last:border-0">
                    <td className="py-2 font-medium text-gray-800">{item.variante.producto?.nombre_base}</td>
